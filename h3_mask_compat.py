"""Capability-aware runtime compatibility for MiniMax H3 AV denoise masks.

Installs missing model-level hooks into comfy.model_base.MiniMaxH3 and
comfy.ldm.minimax.model if native support is absent. No ComfyUI files are
modified on disk.
"""
...
_LOG = logging.getLogger("minimax_creator.mask_compat")
_MARKER = "_mmc_pr15375_compat_v2"
...
def _install_engine_compat(h3m):
    ...
    for fn in (mask_row_values, mod_row, mod_scale_shift, mod_gate, final_forward, h3_forward, h3_inner_forward):
        _mark(fn)
...
def ensure_h3_mask_compat():
    """Install only missing #15375 mask capabilities into the live ComfyUI runtime."""
    import comfy.model_base as model_base
    import comfy.ldm.minimax.model as h3m

    cls = getattr(model_base, "MiniMaxH3", None)
    if cls is None:
        raise RuntimeError("MiniMaxH3 model class not found in comfy.model_base")

    status = capability_status()
    if not status["mask_engine_complete"]:
        _install_engine_compat(h3m)
        _LOG.info("MiniMax-Creator: H3 diffusion-mask engine compatibility enabled")
    ...
    if need_process or need_scale:
        ...
        if need_process:
            ...
            _LOG.info("MiniMax-Creator: H3 mask preprocessing compatibility enabled")
        if need_scale:
            cls.scale_latent_inpaint = scale_fn
            _LOG.info("MiniMax-Creator: H3 inpaint scaling compatibility enabled")
